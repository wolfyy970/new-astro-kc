# KC Wolff-Ingham — Design Leadership Portfolio
...
- **A Ladder, Not a Duplicate:** Marked term → margin note (label, figure, one sentence) → the full note, with a project destination exposed only where one exists. Above 1420px the note simply continues in the margin; between 600px and that threshold it binds into the document flow; on a phone the same note rises as the bottom sheet.
- **A Time Axis:** Experience sets abbreviated start and end dates in a two-line mono rail hanging in the sheet's own margin, so tenure and sequence read at a glance without parsing a sentence. It replaced the company-left/dates-right row — the most generic pattern the résumé form has.
...
- **Accessible Editorial Design:** the automated route audit currently reports no definite violations; incomplete ARIA, contrast, autoplay-audio, and caption checks remain for manual review.
...
- **Content Integrity Suite:** Automated validation of hotspot links, image and media paths, the case-study inventory, and the downloadable résumé to prevent regressions.
...
- **Independent work and writing:** `/projects` presents Org Chart Studio, Unreel Recipes, and Designer in that order, with an in-page contents list, project identity and media, and a dated index linked to KC's _horizon_ publication on Substack. `/references` presents eight validated testimonials.

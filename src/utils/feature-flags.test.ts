...
describe("isCaseStudyLinkEnabled", () => {
  const checkEnabled = (linkPath: string): boolean => {
    const testMap: PopoverMap = {
      testKey: {
        label: "Test",
        text: "Text",
        link: linkPath,
    ...
    const result = applyFeatureFlags(testMap);
    return result.testKey.link !== undefined;
  };
  ...
  it("trims whitespace around slugs in the env var", () => {
    ...
    expect(checkEnabled("/two-way-tv")).toBe(true);
    expect(checkEnabled("/another-slug")).toBe(true);
  });

  it("returns false for a slug not in the enabled list", () => {
    vi.stubEnv("CASE_STUDY_LINKS", "two-way-tv");
    expect(checkEnabled("/some-other-page")).toBe(false);
  });
  ...
  it('returns true for any slug when CASE_STUDY_LINKS is the literal string "true"', () => {
    ...
    expect(checkEnabled("/truist")).toBe(true);
    expect(checkEnabled("/sparks-grove")).toBe(true);
    expect(checkEnabled("/some-future-page")).toBe(true);
  });
